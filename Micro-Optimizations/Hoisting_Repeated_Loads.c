...
static void Conv2d(){
    ...
    const int32_t kernel00 = kernel[0][0], kernel01 = kernel[0][1], kernel02 = kernel[0][2];
    const int32_t kernel10 = kernel[1][0], kernel11 = kernel[1][1], kernel12 = kernel[1][2];
    const int32_t kernel20 = kernel[2][0], kernel21 = kernel[2][1], kernel22 = kernel[2][2];
    ...
    for (int till_y = 0; till_y < out_height; till_y += till_height){
        for (int till_x = 0; till_x < out_width; till_x += till_width){
            ...
            for(int y = 0; y < y_end; y += 1){
                int row_base = till_y + y;
                int32_t *output_row_base = &output[till_y + y][till_x]; 


                for(int x = 0; x < x_end; x += 1){
                    const uint8_t *p0 = &image[row_base + 0][till_x + x];
                    const uint8_t *p1 = p0 + width;
                    const uint8_t *p2 = p1 + width;  
                    

                    int32_t sum = 0;
                    sum += (int32_t)kernel00 * (int32_t)p0[0];
                    sum += (int32_t)kernel01 * (int32_t)p0[1];
                    sum += (int32_t)kernel02 * (int32_t)p0[2];
                    sum += (int32_t)kernel10 * (int32_t)p1[0];
                    sum += (int32_t)kernel11 * (int32_t)p1[1];
                    sum += (int32_t)kernel12 * (int32_t)p1[2];
                    sum += (int32_t)kernel20 * (int32_t)p2[0];
                    sum += (int32_t)kernel21 * (int32_t)p2[1];
                    sum += (int32_t)kernel22 * (int32_t)p2[2];  
   
                    output_row_base[x] = sum;    
                }
            }
        }
    }
